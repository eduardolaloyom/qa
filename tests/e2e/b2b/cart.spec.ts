...
test.describe('C2 — Carrito de compras', () => {
  test.beforeEach(async ({ authedPage: page }) => {
    await page.goto('/products');
    await expect(page.getByRole('button', { name: 'Agregar' }).first()).toBeVisible({ timeout: 30_000 });
  });

  test('C2-05: Agregar producto al carro', async ({ authedPage: page }) => {
  ...
  test('C2-08: Modificar cantidad en carro', async ({ authedPage: page }) => {
    // Agregar producto
    await Promise.all([
      page.waitForResponse(resp => resp.url().includes('/cart') && resp.request().method() === 'POST'),
  ...
  test('C2-09: Eliminar producto del carro', async ({ authedPage: page }) => {
    // Agregar producto
    await Promise.all([
      page.waitForResponse(resp => resp.url().includes('/cart') && resp.request().method() === 'POST'),
      page.getByRole('button', { name: 'Agregar' }).first().click(),
